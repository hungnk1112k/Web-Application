...
<body>
	<h2>EOI database - Search By Job Reference Number</h2>
<?php
	if (!isset ($_POST["Job_Reference_Number"])) {
		header("Location: home.php");
	} else {
		$jobnum = trim(htmlspecialchars($_POST["Job_Reference_Number"]));
		...
		if (!$conn) {
			...
			$query = " select * from $sql_table where Job_Reference_Number like '$jobnum'";
			...
			if(!$result) {
				...
				echo "</table>\n ";
					...
					echo "<p style='font-size: 120%'>No record found for Job Reference Number $jobnum!</p>";
				}
